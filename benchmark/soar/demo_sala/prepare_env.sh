...
if [[ "$QUANT_MODE" == "gptq" ]]; then
	...
	export SGLANG_SERVER_ARGS="${SGLANG_SERVER_ARGS:-} --trust-remote-code --disable-radix-cache --attention-backend minicpm_flashinfer --chunked-prefill-size 32768 --max-prefill-tokens 32768 --prefill-max-requests 1 --max-running-requests 20 --mem-fraction-static 0.84 --schedule-conservativeness 1.0 --dense-as-sparse --quantization gptq_marlin --force-dense-minicpm --kv-cache-dtype fp8_e5m2 --speculative-algorithm NGRAM --speculative-num-draft-tokens 12${FUSED_QK_NORM_ROPE_ARG}"
fi
